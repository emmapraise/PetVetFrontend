...
function NavBar({ searchInput, modalRegister, modalLogin }) {
  ...
  useEffect(() => {
      localStorage.getItem('user') === null ? setIsLogin(false) : setIsLogin(true);
	...
	return (
		<Wrapper>
			<div className="desktop">
				<div className="flex j-btw ">
					<Link to="/">
						...
						<img
							className="logo"
							src="https://res.cloudinary.com/ddxyjg96x/image/upload/v1668759198/PetVet/3cd418089f054af9b1e8c65c4daf89ca.png"
							alt="logo"
							height="200px"
						/>
					</Link>
          ...
          {isLogin ? (
            <>
						<Link to="/dashboard" id="dashboard-nav" className="pd-9">
              ...
              <img src={calenderBookings} alt="cartIcon" className="icon w-19" />
							...
							<button className=" orange btn" onClick={handleLogout}>
								Logout
							</button>
              </>
			...
			<div className="mobile">
				...
				>
					...
					<div className=" flex nav-buttons">
						...
						{/* <Link to="/cart" className="ml-55 mb-10"> */}
						<button className=" orange btn">Logout</button>
